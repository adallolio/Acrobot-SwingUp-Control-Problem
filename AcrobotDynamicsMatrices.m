...
if nargin == 1
    ...
    M = [a1 + 2*a2*cos(acr.q2), a3 + a2*cos(acr.q2);
        a3 + a2*cos(acr.q2), a3];
    C = [-a2*sin(acr.q2)*(acr.q2d^2+2*acr.q1d*acr.q2d);
        a2*sin(acr.q2)*acr.q1d^2];

    G = acr.g0*[(acr.m1*acr.lc1 + acr.m2*acr.l1)*cos(acr.q1) + acr.m2*acr.lc2*cos(acr.q1+acr.q2);
            acr.m2*acr.lc2*cos(acr.q1+acr.q2)];
else 
	q1 = vars(1);
	q1d = vars(3);
	q2 = vars(2);
